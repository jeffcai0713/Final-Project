...
library(ggplot2)
library(readxl)
NBAStat <- read_excel("~/Desktop/Computational Biology & Bioinformatics/NBAStat.xlsx")
View(NBAStat)
...
ggplot(NBAStat, aes(x = pts, y = mp)) +
geom_point(alpha = .5) + geom_density_2d(h=2) + coord_fixed() + facet_wrap(~pos)

man1<-manova(cbind(pts,mp)~pos, data=NBAStat)
summary(man1)
summary.aov(man1) 

NBAStat%>%group_by(pos)%>%summarize(mean(pts),mean(mp))

pairwise.t.test(NBAStat$pts,
NBAStat$pos, p.adj="none")

pairwise.t.test(NBAStat$mp,
NBAStat$pos, p.adj="none")
...
*Ho: NBA point guard's average similar points per game (PPG) as shooting guards.*
*HA: NBA Point guards and shotting guards have different PPG averages.*



PG <- NBAStat%>%filter(grepl("PG",pos ))
SG <- NBAStat%>%filter(grepl("SG",pos ))

PG1 <- mean(PG$pts)
SG1 <- mean(SG$pts)

cric<-data.frame(Position=c(rep("PG",1),rep("SG",1)),PPG=c(PG1,SG1))
head(cric)

ggplot(cric,aes(PPG,fill=Position))+geom_histogram(bins=6.5)+facet_wrap(~Position)

cric1 <- data.frame(Position=c(rep("PG",112),rep("SG",130)),PPG=c(PG$pts,SG$pts))
head(cric1)

ggplot(cric1,aes(PPG,fill=Position))+geom_histogram(bins=6.5)+facet_wrap(~Position)
...
PGvsSG <- rbind(PG3, SG3)

fit<-lm(pts ~ pos * mp, data=PGvsSG)

summary(fit)

center <- mean(PGvsSG$mp)
frame <- data.frame(mpcenter=PGvsSG$mp-center)

PGvsSG$mpc <- PGvsSG$mp - mean(PGvsSG$mp)

fit1<-lm(pts ~ pos * mpc, data=PGvsSG)
summary(fit1)

qplot(x = pts, y = mp, color = pos, data = PGvsSG) +
...
boot_dat<- sample_frac(PGvsSG, replace=T)

samp_distn<-replicate(5000, {
boot_dat <- sample_frac(PGvsSG, replace=T)
fit2 <- lm(pts~ pos+mpc, data=boot_dat)
coef(fit2)
})
...
fitt2<-glm(Position~pts+mp, data=PGorSG, family="binomial")
prob <- predict(fitt2,type="response")
table(predict=as.numeric(prob>.5),yes=PGorSG$Position)%>%addmargins

NBAdiag<-function(prob,yes){
...
NBAdiag(prob,PGorSG$Position)

install.packages("plotROC")
library(plotROC)

RocPlot <- ggplot(PGorSG)+geom_roc(aes(d=Position,m=prob),n.cut=0)
RocPlot
calc_auc(RocPlot)
...
set.seed(1234)
k=10
NBAdata<-PGorSG[sample(nrow(PGorSG)),] 
folds<-cut(seq(1:nrow(PGorSG)),breaks=k,labels=FALSE) 
diags<-NULL
for(i in 1:k){
train<-NBAdata[folds!=i,]
test<-NBAdata[folds==i,]
yes<-test$Position

NBAfit1<-glm(Position~pts*mp,data=PGorSG,family="binomial")
probs2<-predict(NBAfit1,newdata = test,type="response")
